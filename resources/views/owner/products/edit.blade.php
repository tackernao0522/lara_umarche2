            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <div class="-m-2">
                            ...
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative">
                                    ...
                                    <textarea id="information" name="information" rows="10"
                                        class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-purple-500 focus:bg-white focus:ring-2 focus:ring-purple-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">{{ old('information', $product->information) }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative flex justify-around">
                                    ...
                                    <div><input type="radio" name="type"
                                            value="{{ \Constant::PRODUCT_LIST['reduce'] }}" class="mr-2" checked>削減
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <optgroup label="{{ $category->name }}">
                                                @foreach ($category->secondaries as $secondary)
                                                    <option value="{{ $secondary->id }}"
                                                        {{ old('category', $product->shop_id) == $secondary->id ? 'selected' : '' }}>
                                                        {{ $secondary->name }}
                                                    </option>
                                                @endforeach
                                            </optgroup>
                            ...
                            <div class="p-2 w-1/2 mx-auto">
                                <div class="relative flex justify-around">
                                    <div><input type="radio" name="is_selling" value="1"
                                            {{ old('is_selling', $product->is_selling) == 1 ? 'checked' : '' }}
                                            class="mr-2">販売中</div>
                    ...
                    <form id="delete_{{ $product->id }}"
                        action="{{ route('owner.products.destroy', $product->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
